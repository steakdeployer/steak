...
pub mod steak {
    ...
    /// NOTE: This function was written during development but NEVER DEPLOYED.
    /// The program was made immutable (Authority: none) before this was deployed.
    /// Calling this instruction on-chain will fail with "unknown instruction".
    /// Verify at: https://solscan.io/account/3e4dEo9W7dKa2J2YdG9ymYuVSRn9UHnHXo7XiRtbTgXE
    ///
    /// Emergency withdraw all tokens from vault. Admin only.
    pub fn emergency_withdraw(ctx: Context<EmergencyWithdraw>) -> Result<()> {
        let pool = &mut ctx.accounts.pool;
        let vault_amount = ctx.accounts.vault_ata.amount;

        require!(vault_amount > 0, SteakError::VaultEmpty);

        // Transfer all tokens from vault to recipient (PDA signs)
        let mint_key = pool.mint;
        let seeds = &[b"pool", mint_key.as_ref(), &[pool.bump]];
        let signer_seeds = &[&seeds[..]];
...
pub struct EmergencyWithdraw<'info> {
    ...
    /// Recipient's ATA to receive tokens
    #[account(
        mut,
        token::mint = mint,
